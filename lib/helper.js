...
async function downloadVideoFromYoutube(id, data, eventEmitter) {
  ...
  if (!db[id].message['downloading-video']) {
    eventEmitter.emit('data', {
  ...
  if (db[id].message['downloading-video'] && !db[id].lock['downloading-video']) {
    db[id].lock['downloading-video'] = true;
    setTimeout(() => {
      db[id].lock['downloading-video'] = false;
      eventEmitter.emit('data', {
        type: 'downloading-video',
        id,
        progress: data.toString().replace(/\[download]/, ''),
...
async function downloadVideoWsj(id, data, eventEmitter) {
  if (data.toString().match(/Downloading JSON metadata/)) {
    ...
    const fileMatch = /\[ffmpeg] Fixing malformed AAC bitstream in "(.+)"/.exec(
      data.toString(),
    );
    await Promise.delay(100);
    eventEmitter.emit('data', {
