...
class Output {
  ...
  async _sendDownloadingVideo() {
    ...
    if (
      ...
      db[this.data.id].last['downloading-video']
    ) {
      db[this.data.id].last['downloading-video'] = this.data.progress;
      return;
    }
    ...
    const interval = setInterval(async () => {
      ...
      const progress = db[this.data.id].last['downloading-video'] || '';
      body.text = `Downloading ${db[this.data.id].part['downloading-video']} video (${this.data.id}): ${progress}`;
